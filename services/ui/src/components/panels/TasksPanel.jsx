    ...
    gap: 12
      ...
      gap: 12
        ...
        return <div key={t.id} style={{
          ...
          background: '#f9f9f9'
            ...
            textAlign: 'center',
            marginBottom: 2
          }}>Transporter {t.id}</div>
                      <div style={{
            display: 'flex',
            gap: 4
          }}>
                        <input type="number" value={inputs.lift} onChange={e => setTaskInputs(prev => ({
              ...prev,
              [t.id]: {
                ...inputs,
                lift: e.target.value
              ...
              fontSize: 11
                        ...
                        <input type="number" value={inputs.sink} onChange={e => setTaskInputs(prev => ({
              ...prev,
              [t.id]: {
                ...inputs,
                sink: e.target.value
              }
            }))} placeholder="Sink" style={{
              flex: 1,
              padding: '4px 6px',
              borderRadius: 4,
              border: '1px solid #ccc',
              fontSize: 11
                        ...
                        <button onClick={() => handleSendTask(t.id, inputs.lift, inputs.sink)} style={{
              padding: '4px 12px',
              borderRadius: 4,
              ...
              {manualTasks.length > 0 && <div style={{
      padding: 8,
      ...
      background: '#fff3e0'
        ...
        gap: 4
                    ...
                    {manualTasks.map(mt => <div key={mt.id} style={{
          display: 'flex',
          alignItems: 'center',
          ...
          border: '1px solid #ffcc80'
                        ...
                        <span>
                          ...
                          {' '}
                          <span style={{
              color: mt.status === 'fitted' ? '#2e7d32' : '#bf360c',
              fontWeight: 600
            }}>
                            [{mt.status}]
                          </span>
                          {mt.fitted_start_s != null && <span style={{
              color: '#666',
              marginLeft: 6
            }}>
                        ...
                        <button onClick={() => handleCancelManualTask(mt.id)} style={{
            padding: '2px 8px',
            borderRadius: 3,
      ...
      gap: 12
        ...
        return <div key={`exec-${t.id}`} style={{
          flex: 1,
          padding: 8,
      ...
      gap: 12
        ...
        const tasksForTransporter = transporterTasks.filter(task => task.transporter_id === t.id).sort((a, b) => a.task_start_time - b.task_start_time);
        return <div key={`tasks-${t.id}`} style={{
          flex: 1,
          ...
          background: '#fff'
            ...
            overflowY: 'auto'
                        ...
                        {tasksForTransporter.length === 0 ? <div style={{
              padding: '4px 0',
              fontStyle: 'italic',
              color: '#666'
            }}>No tasks</div> : <table style={{
              width: '100%',
              borderCollapse: 'collapse'
                  ...
                  return <tr key={idx} style={{
                    ...
                    borderBottom: idx < tasksForTransporter.length - 1 ? '1px solid #f0f0f0' : 'none'
                      ...
                      fontSize: '10px'
                    }}>{task.lift_station_id}</td>
                                    <td style={{
                      textAlign: 'right',
                      padding: '2px 4px',
                      fontFamily: 'monospace',
                      fontSize: '10px'
                    }}>{task.sink_station_id}</td>
                                    <td style={{
                      textAlign: 'right',
                      padding: '2px 4px',
                      fontFamily: 'monospace',
      ...
      background: '#fff'
        ...
        overflowY: 'auto'
                  ...
                  {transporterTasks.filter(task => task.transporter_id === null).length === 0 ? <div style={{
          padding: '4px 0',
          fontStyle: 'italic',
          color: '#666'
        }}>No unassigned tasks</div> : <table style={{
          width: '100%',
          borderCollapse: 'collapse'
        }}>
                      <thead>
              ...
              borderBottom: '2px solid #ddd'
                ...
                color: '#666'
                      ...
                      <tbody>
                        {transporterTasks.filter(task => task.transporter_id === null).sort((a, b) => a.task_start_time - b.task_start_time).map((task, idx) => {
              const now = elapsedMs;
              const relStart = Math.round(task.task_start_time - now);
