...
export default function App() {
  ...
  const [simPurposeForm, setSimPurposeForm] = useState({
    country: '',
    city: '',
    purpose: ''
  });
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        const [configRes, transportersRes, stationsRes, tanksRes] = await Promise.allSettled([api.get('/api/config/layout_config.json'), api.get('/api/config/transporters.json'), api.get('/api/config/stations.json'), api.get('/api/config/tanks.json')]);
        ...
        const tanksData = tanksRes.status === 'fulfilled' ? tanksRes.value : {
          tanks: []
        };
  ...
  const generateInitialStates = (transportersList, stationsList) => {
    const initialStates = transportersList.filter(t => t.model === "2D").map(transporter => {
      const startStation = stationsList.find(s => s.number === transporter.start_station);
      return {
        id: transporter.id,
  ...
  const fetchUnitsFromApi = async () => {
    try {
      const data = await api.get(`/api/units?ts=${Date.now()}`, {
        cache: 'no-store'
      });
  ...
  const applyProductionDefaults = (incoming, stationsList = [], previous = null) => {
    ...
    const loadingStations = Array.isArray(stationsList) ? stationsList.filter(s => (s?.name || '').toLowerCase() === 'loading') : [];
    const unloadingStations = Array.isArray(stationsList) ? stationsList.filter(s => (s?.name || '').toLowerCase() === 'unloading') : [];
  ...
  const handleAvoidStatusChange = async (stationNumber, newStatus) => {
    try {
      await api.post(`/api/avoid-statuses`, {
        stationNumber: String(stationNumber),
        avoid_status: newStatus
      });
      await loadAvoidStatuses();
    } catch (err) {
  ...
  const handleUnitSave = async () => {
    ...
    if (!Number.isFinite(newLoc)) {
      setBatchError('Invalid location');
      return;
    }
    setUnitSaving(true);
    setBatchError('');
  ...
  const handleBatchSubmit = async e => {
    ...
    const url = editingBatchId != null ? `/api/batches/${editingBatchId}` : '/api/batches';
    try {
      if (method === 'PUT') {
  ...
  const handleCreateProduction = async () => {
    ...
    if (productionRows.length === 0) {
      ...
      if (productionPrograms.length > 0) {
        const defaultRow = {
          program: Number(productionPrograms[0]),
          count: 1,
          start_station: '',
          end_station: ''
        };
  ...
  const submitProductionPairs = async (pairs, validPrograms) => {
    ...
    try {
      ...
      const data = await api.post(`/api/production`, {
        pairs
      });
      if (!data || !data.success) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      await api.post('/api/production-queue', {
        value: 1
      });
  ...
  const handleSendTask = async (transporterId, liftValue, sinkValue) => {
    ...
    try {
      ...
      const data = await api.post(`/api/command/move`, {
        transporterId,
        lift_station: liftNumber,
        sink_station: sinkNumber
      });
      if (data && data.queued) {
  ...
  const handleTogglePlc = async () => {
    setPlcToggling(true);
    const isRunning = plcStatus?.runtime_status === 'running';
    try {
      if (isRunning) {
        await api.post('/api/plc/stop');
        setTimeout(async () => {
          try {
            const d = await api.get('/api/plc/status');
            if (d) setPlcStatus(d);
          } catch {}
          setPlcToggling(false);
        }, 3000);
      } else {
        await api.post('/api/plc/start');
        setTimeout(async () => {
          try {
            const d = await api.get('/api/plc/status');
            if (d) setPlcStatus(d);
  ...
  // formatTime is extracted to utils/formatTime.js
  ...
  const plcBatches = useMemo(() => {
    ...
    return units.filter(u => u.batch_code && u.batch_code > 0).map(u => ({
  ...
  return <div className="app">
      ...
      {debugTransporterState && debugTransporter && <DraggablePanel onClose={() => setDebugTransporterId(null)} title={`Transporter ${debugTransporter.id} – debug`}>
        ...
        color: '#333'
          ...
          {/* Calibration Panel */}
          {showCalibration && <CalibrationPanel onClose={() => setShowCalibration(false)} stations={stations} transporterStates={transporterStates} />}

          <ConfigPanel showConfig={showConfig} setShowConfig={setShowConfig} configForm={configForm} setConfigForm={setConfigForm} configError={configError} setConfigError={setConfigError} configSaving={configSaving} setConfigSaving={setConfigSaving} config={config} setConfig={setConfig} />


          {showCustomer && <CustomerPanel {...appContextProps} />}

          {showProduction && <ProductionPanel {...appContextProps} />}

        {showBatches && <UnitsPanel {...appContextProps} />}


        {showTasks && <TasksPanel {...appContextProps} />}
    </div>;
}